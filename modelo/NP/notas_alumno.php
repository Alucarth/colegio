        ...
        <div class="modal-content">
			...
			?>              
			  ...
			  ?>
                  <div class="col-md-3">
                      <div class="form-group">
                          <label>Primer Bimestre: <?php 
						  if($base->dex_query_num_rows("select estado_n from $tabla_e where estado_n='1'")==1){
							 ?><span class="text-danger">(Requerido)</span><?php }
						  else{?>
                          <span class="text-success">(Bloquedo)</span><?php }?></label>
                          <input type="text" name="primero" class="form-control <?php 
						  if($base->dex_query_num_rows("select estado_n from $tabla_e where estado_n='1'")==1){
							  ?>required" id="nota_1" placeholder="Escriba la primero nota" <?php }
						  else{?> id="primero" placeholder="Nota Deshabilitada" disabled<?php }?> />
                      </div>
                  </div> 
