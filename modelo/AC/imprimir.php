...
$db = new MySQL(); 
$i=0;
$paralelo = $db->consulta("select * from actividad where id_mes ='".$_GET["valor"]."'");
  while($resultados = $db->fetch_array($paralelo))
  {			
	  $dia[$i]=$resultados["dia"];
	  $titulo[$i]=$resultados["titulo"];	  
	  $descripcion[$i]=$resultados["descripcion"];
	  $i++;
  }  
...
$pdf->SetFontSize(14);
$pdf->Cell(0,0,'Lista de Actividades de Mes',0,1,'C');		
$pdf->Ln(10);
			$pdf->SetTextColor(0);
			...
			$pdf->SetWidths(array(10,40,50,70));
			$pdf->SetTextColor(253, 253, 255);
			$pdf->SetAligns(array('C','C','C','C')); 
			$pdf->Row(array('N','Dia','titulo de Actividad','Descripcion'));
			$n=1;
			for($j=0;$j<$i;$j++)
			{
			  ...
			  $pdf->Row(array($n,$dia[$j],$titulo[$j],$descripcion[$j]));
			  $n++;
			}
$pdf->Output();
?>		