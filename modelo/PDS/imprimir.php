...
$db = new MySQL(); 
$i=0;
$docente = $db->consulta("select * from docente_s");
  while($resultados = $db->fetch_array($docente))
  {			
	  $ci[$i]=$resultados['carnet'];
	  $nombre[$i]=$resultados['nombre']." ".$resultados["ap_paterno"]." ".$resultados["ap_materno"];
	  $mat[$i]=$resultados["materia"];
	  $i++;
  }  
...
$pdf->SetFontSize(14);
$pdf->Cell(0,0,'Lista de Docentes de Secundaria',0,1,'C');		
$pdf->Ln(10);
			$pdf->SetTextColor(0);
			...
			$pdf->Row(array('N','Carnet de identidad','Docente','Materia'));
			$n=1;
			for($j=0;$j<$i;$j++)
			{
			  ...
			  $pdf->Row(array($n,$ci[$j],$nombre[$j],$mat[$j]));
			  $n++;
			}
$pdf->Output();
?>		