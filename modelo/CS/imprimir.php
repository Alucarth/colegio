...
$db = new MySQL(); 
$i=0;
$paralelo = $db->consulta("select * from secundaria_aulas where id_secundaria ='".$_GET["valor"]."'");
  while($resultados = $db->fetch_array($paralelo))
  {			
	  $nombre[$i]=$resultados['paralelo'];
	  $cantidad[$i]=$resultados['cantidad'];
	  $i++;
  }  
...
$pdf->SetFontSize(14);
$pdf->Cell(0,0,'Lista de Paralelos de Secundaria',0,1,'C');		
$pdf->Ln(10);
			$pdf->SetTextColor(0);
			...
			$pdf->Row(array('N','Paralelo','Cantidad Maxima de Alumnos'));
			$n=1;
			for($j=0;$j<$i;$j++)
			{
			  ...
			  $pdf->Row(array($n,$nombre[$j],$cantidad[$j]));
			  $n++;
			}
$pdf->Output();
?>		